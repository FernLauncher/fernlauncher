...
import "node:events";
import "node:stream";
import require$$1$2, { execSync as execSync$1, spawn } from "child_process";
import fs$k from "fs";
...
function getJavaVersion(javaBin) {
  var _a, _b;
  try {
    const output = execSync$1(`"${javaBin}" -version 2>&1`, {
      timeout: 5e3,
      stdio: "pipe"
    }).toString();
...
function getArchitecture(javaBin) {
  try {
    const output = execSync$1(`"${javaBin}" -XshowSettings:all -version 2>&1`, { timeout: 5e3 }).toString();
